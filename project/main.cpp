...
std::vector<int> histograma_serial(int *arr, int n, int grupos) {
    ...
    int intervalo = std::ceil((double)(max - min)/grupos);
    ...
    for (int i = 0; i < n; i++) {
        int indice = (arr[i] - min) / intervalo;
...
#pragma omp parallel for reduction(min:min) reduction(max:max) default(none) shared(arr, n)
    for (int i = 0; i < n; ++i) {
        if(arr[i] < min){
            min = arr[i];
        }
...
#pragma omp parallel default(none) shared(arr, n, res, grupos, intervalo, min)
    {
...
std::vector<int> histograma_omp(int *arr, int n, int grupos) {
    ...
    // Encontrar el mínimo y máximo en paralelo
...
std::vector<int> histograma_cuda(int *arr, int n, int grupos) {
    ...
    invoke_histograma_cuda(d_arr, d_res, n, grupos, min, intervalo);
